...
Description:
Converts raw flat-panel detector frames into polar cake plots and azimuthally integrates them to produce 1D diffraction profiles.
"""
"""
...
Description:
This script performs automated conversion and processing of 2D X-ray diffraction (XRD) data.
It converts proprietary .gfrm files to HDF5 format, extracts metadata, and generates 
azimuthal cake plots, integrated profiles, and peak analysis visualisations. All output
data and visualisations are automatically exported to a dynamically generated, 
timestamped output directory based on the sample name.
...
conda install numpy h5py matplotlib scipy pandas imageio Pillow
...
def auto_contrast(image, lower=1, upper=99):
    """
    Calculates the minimum and maximum values for image display contrast
    based on the specified percentiles.

    Parameters:
        image (numpy.ndarray): The 2D array representing the detector image.
        lower (float): The lower percentile bound (default: 1).
        upper (float): The upper percentile bound (default: 99).

    Returns:
        tuple: Minimum and maximum intensity values (vmin, vmax) for plotting.
    """
...
def load_reference_peaks(filepath, min_intensity=2.0):
    """
    Parses crystallographic reference data from a standard text file format.
    Extracts the 2Theta peak position, intensity, Miller indices (h, k, l),
    and d-spacing for reflections that exceed a minimum intensity threshold.
        Parameters:
    filepath (str): The absolute or relative path to the reference text file.
    min_intensity (float): The minimum relative intensity required for a 
    reflection to be included in the analysis.
        Returns:
    list: A list of tuples containing (two_theta, intensity, h, k, l, d_spacing).
    """
    ref_peaks = []
...
def process_single_file(input_gfrm, timestamp, calcite_refs, vaterite_refs):
    ...
    output_dir = os.path.join("data/processed/2D_XRD/output", f"{sample_name}_{timestamp}")
    ...
    with pd.ExcelWriter(excel_path, engine='xlsxwriter') as writer:
        ...
        # Robust morphological baseline to prevent negative profiles
        window_size = 151  # Approx 3 degrees wide for 0.02 step
        ...
        # Remove minor negative dips caused by smoothing artifacts
        ...
        # Dynamic prominence threshold to adapt to any sample intensity (2% of max peak)
        # Using std deviation is skewed by massive peaks, so a simple percentage is more robust
        ...
        # Plot 4: Calcite Only (if matched)
        if len(calcite_matched) > 0:
            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), sharex=True)
            ax1.imshow(cake_plot, extent=[theta_lin.min(), theta_lin.max(), phi_lin.min(), phi_lin.max()],
                       aspect='auto', origin='lower', cmap='inferno', vmin=vmin, vmax=vmax)
            ax1.set_ylabel('Azimuthal Angle φ (degrees)')
            ax1.set_title('Cake Plot with Detected Peaks (Calcite Only)')
            ax2.plot(theta_lin, integrated_profile, color='blue', alpha=0.4, label='Integrated Profile')
            ax2.plot(theta_lin, corrected_profile, color='darkorange', label='Corrected Profile')
            ax2.plot(theta_lin[peaks], corrected_profile[peaks], 'rx', label='Detected Peaks')
            for peak_pos in theta_lin[peaks]:
            ...
            plt.savefig(plot_4_path, dpi=100)
            plt.close()
            images_for_gif.append(plot_4_path)

        # Plot 5: Vaterite Only (if matched, or if nothing matched Calcite)
        if len(vaterite_matched) > 0 or len(calcite_matched) == 0:
            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), sharex=True)
            ax1.imshow(cake_plot, extent=[theta_lin.min(), theta_lin.max(), phi_lin.min(), phi_lin.max()],
                       aspect='auto', origin='lower', cmap='inferno', vmin=vmin, vmax=vmax)
            ax1.set_ylabel('Azimuthal Angle φ (degrees)')
            ax1.set_title('Cake Plot with Detected Peaks (Vaterite Only)')
            ax2.plot(theta_lin, integrated_profile, color='blue', alpha=0.4, label='Integrated Profile')
            ax2.plot(theta_lin, corrected_profile, color='darkorange', label='Corrected Profile')
            ax2.plot(theta_lin[peaks], corrected_profile[peaks], 'rx', label='Detected Peaks')
            for peak_pos in theta_lin[peaks]:
            ...
            ax2.legend()
            plt.tight_layout()
            plot_5_path = os.path.join(output_dir, 'plot_5_vaterite_only.png')
            plt.savefig(plot_5_path, dpi=100)
            plt.close()
            images_for_gif.append(plot_5_path)
        ...
        for idx, peak_pos in enumerate(theta_lin[peaks]):
            ...
            # 
            # Physical Interpretation of Azimuthal Fluctuations:
            # The diffraction rings often exhibit significant intensity fluctuations 
            # (graininess or spottiness) along the azimuthal angle. When overall intensity 
            # is high, this phenomenon is not a result of Poisson noise but characterises 
            # the microstructural morphology of the analysed thin film. 
            # 
            # It typically indicates either:
            # 1. Poor powder statistics: The film comprises distinct, larger crystallites 
            #    (micro-scale rather than nano-scale). These large grains act as individual 
            #    diffracting centres, producing discrete, intense spots rather than a 
            #    continuous, isotropic powder ring.
            # 2. Localised columnar growth: Even if macroscopic texture is weak (low Degree 
            #    of Anisotropy), local domains of highly ordered columnar crystallisation 
            #    can produce intense, localised diffraction spots at specific azimuthal angles.
            # 
            # The Coefficient of Variation (CV) metric is explicitly utilised here to quantify 
            # this graininess. A perfectly isotropic, fine-grained powder will yield a CV 
            # near zero, whereas a film with large or columnar domains will yield a proportionally 
            # higher CV.
            # 
            # Note on Distinguishing Columnar from Spiral Growth:
            # A single stationary 2D XRD frame (which measures a single slice of 
            # reciprocal space) is insufficient to definitively distinguish between 
            # straight columnar growth and spiral (chiral) columnar growth. While 
            # spiralling lattices may introduce azimuthal smearing, simple mosaic spread 
            # produces an identical effect. Conclusive evidence of spiralling requires 
            # calculating a full 3D pole figure—which necessitates physically rotating 
            # the sample (tilt and azimuthal sweeps) during measurement—or using direct 
            # cross-sectional electron microscopy (e.g., SEM/TEM).
    ...
    """)
        if len(vaterite_matched) > 0 or len(calcite_matched) == 0:
            f.write(f"""
        <div class="image-container">
            <img src="plot_5_vaterite_only.png" alt="Vaterite Only Plot">
        </div>
    """)
...
if __name__ == "__main__":
    ...
    parser.add_argument('input_path', nargs='?', default='data/raw/2D_XRD', help='Path to .gfrm file or directory')
    ...
    # Run in parallel using the number of available CPU cores
    ...
    if comparison_data:
        comp_dir = os.path.join("data/processed/2D_XRD/output", f"comparison_{timestamp}")
