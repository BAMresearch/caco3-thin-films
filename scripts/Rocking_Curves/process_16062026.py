# -*- coding: utf-8 -*-
"""
Rocking Curve Processor for 16 June 2026 Run
============================================
...
Description:
Processes and fits rocking curve profiles from the 16 June 2026 measurement batch.
"""
import os
import zipfile
...
raw_data_dir = os.path.join(base_dir, "data/raw/Rocking_Curves/SH-125-G")
processed_dir = os.path.join(base_dir, "data/processed/Rocking_Curves/SH-125-G")
analysis_dir = os.path.join(base_dir, "data/processed/Rocking_Curves/16062026")
...
def extract_brml_data(brml_path):
    if not os.path.exists(brml_path) or os.path.getsize(brml_path) == 0:
        raise FileNotFoundError(f"Missing or empty file: {brml_path}")
...
# Peak configuration for SH-125-G (similar to SH-125-A, as it has a similar Calcite/Vaterite signature)
peaks_config = [
...
# Detect all unique Phi values from filenames
files = os.listdir(raw_data_dir)
phi_values = sorted(list(set([int(f.split("_")[1].split(".")[0]) for f in files if (f.startswith("2Theta_") or f.startswith("Rocking_")) and f.endswith(".brml")])))
print(f"Detected unique Phi values in data folder: {phi_values}")
...
for phi in phi_values:
    ...
    # 1. Process 2Theta scan if present
    ...
    if os.path.exists(twotheta_path):
        ...
        print(f"  Warning: 2Theta file missing for Phi = {phi}°")
        t0_c = 29.4
        
    # 2. Process Rocking Curve if present
    ...
    if os.path.exists(rocking_path):
        ...
        try:
            ...
            for i, p in enumerate(peaks_config):
                ...
                peak_fits.append({
                    "name": p["name"],
                    "center": t0,
                    "tilt": t0 - t0_c/2 if 't0_c' in locals() else t0 - 14.7,
                    "fwhm": fwhm,
                    "height": h,
                    "area": area,
                    "ratio": ratio,
            ...
            for p in peaks_config:
                peak_fits.append({
                    ...
                    "tilt": p["init_center"] - t0_c/2 if 't0_c' in locals() else p["init_center"] - 14.7,
                    "fwhm": 0.0,
                    "height": 0.0,
                    "area": 0.0,
                    "ratio": 0.0,
        ...
        print(f"  Warning: Rocking Curve file missing for Phi = {phi}°")
...
factor = 1.0
active_rocking_phis = sorted(phi_rocking_data.keys())
for phi in active_rocking_phis:
    data = phi_rocking_data[phi]
...
for phi in active_rocking_phis:
    ...
    plt.plot(data["theta"], full_model * factor, 'k-', linewidth=1.2, label=f"Phi = {phi}° Fit" if phi == active_rocking_phis[0] else "")
...
if has_vaterite:
    vaterite_areas = [phi_twotheta_data[p]["vaterite"][3] / 1e3 if phi_twotheta_data[p]["vaterite_detected"] else 0.0 for p in phi_list]
...
theta_0_dict = {phi: phi_twotheta_data[phi]["calcite"][1] / 2.0 if phi in phi_twotheta_data and phi_twotheta_data[phi]["calcite"][3] > 0 else 14.7 for phi in phi_values}
rocking_scans = {}
for phi in active_rocking_phis:
...
step_offset = 4000
for idx, phi in enumerate(active_rocking_phis):
    csv_path = os.path.join(sample_processed_dir, f"{sample}_corrected_rocking_{phi}.csv")
    df_c = pd.read_csv(csv_path)
    theta_arr = df_c["Theta (degrees)"]
...
print(f"\nData processing for {sample} completed successfully!")
